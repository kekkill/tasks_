...
class Program
{
    ...
    private static bool ContainsOnlyRussianLetters(string word)
    {
    ...
    private static bool ContainsOnlyEnglishLetters(string word)
    {
        for (int i = 0; i < word.Length; i++)
        {
            char c = word[i];
    ...
    private static void StartTimer()
    {
        ...
        _timer = new Timer(TimerCallback, null, TIME_LIMIT_MS, Timeout.Infinite);
    }
    ...
    private static void InitializeGame()
    {
        ...
        string languageErrorMsg = LocalWord("LanguageError");

        bool isValidWord = false;
        while (!isValidWord)
        {
            if (_startWord.Length < MIN_WORD_LENGTH || _startWord.Length > MAX_WORD_LENGTH)
            {
                Console.Write(errorMsg);
                _startWord = Console.ReadLine().ToLower().Trim();
            }
            else if (_currentCulture.Name == "ru-RU" && !ContainsOnlyRussianLetters(_startWord))
            {
                Console.Write(languageErrorMsg);
                _startWord = Console.ReadLine().ToLower().Trim();
            }
            else if (_currentCulture.Name == "en-US" && !ContainsOnlyEnglishLetters(_startWord))
            {
                Console.Write(languageErrorMsg);
                _startWord = Console.ReadLine().ToLower().Trim();
            }
    ...
    private static string ValidateWord(string word)
    {
        ...
        if (_currentCulture.Name == "ru-RU" && !ContainsOnlyRussianLetters(word))
        {
            return LocalWord("LanguageError");
        }
        else if (_currentCulture.Name == "en-US" && !ContainsOnlyEnglishLetters(word))
        {
            return LocalWord("LanguageError");
        }
